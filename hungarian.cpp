...
Hungarian::Hungarian() {
  ...
  m_costmatrix.resize(m_rows, std::vector<int>(m_cols, 0));
  m_assignment.resize(m_rows, std::vector<int>(m_cols, 0));
}

Hungarian::Hungarian(const std::vector<std::vector<int>> &input_matrix,
  ...
  m_costmatrix.resize(rows, std::vector<int>(cols, 0));
...
void hungarian_print_matrix(const std::vector<std::vector<int>> &C, int rows,
                            int cols) {
  int i, j;
  fprintf(stderr, "\n");
  for (i = 0; i < rows; i++) {
...
int Hungarian::init(const std::vector<std::vector<int>> &input_matrix, int rows,
                    int cols, MODE mode) {
...
bool Hungarian::check_solution(const std::vector<int> &row_dec,
...
bool Hungarian::assign_solution(const std::vector<int> &row_dec,
                                const std::vector<int> &col_inc,
                                const std::vector<int> &col_vertex) {
...
bool Hungarian::solve() {
  ...
  std::vector<int> col_vertex(m), row_vertex(n), unchosen_row(m), parent_row(n),
  ...
  m_assignment.assign(m, std::vector<int>(n, HUNGARIAN_NOT_ASSIGNED));
...
const std::vector<std::vector<int>> &Hungarian::assignment() const {
  return m_assignment;
}
